...
export const SalesReturnModal: React.FC<SalesReturnModalProps> = ({
  ...
  const [invoiceData, setInvoiceData] = useState<ReturnableInvoiceData | null>(null);

  // Form State
  const [returnDate, setReturnDate] = useState<string>(
    new Date().toISOString().split("T")[0]
  );
  const [refundMethod, setRefundMethod] = useState<"cash" | "credit" | "bank_transfer">("cash");
  const [reason, setReason] = useState<string>("");
  const [lines, setLines] = useState<ReturnLineState[]>([]);

  // Load returnable lines when modal opens
  useEffect(() => {
    ...
    const loadData = async () => {
      ...
      setError(null);
      try {
        const data = await salesApi.getReturnableLines(invoiceId);
        setInvoiceData(data);
        setRefundMethod(
          data.payment_method === "credit" ? "credit" : "cash"
        );

        const mappedLines: ReturnLineState[] = data.lines.map((l) => ({
          ...
          quantity_to_return: 0, // initially 0
  ...
  // Update line return quantity
  const handleQuantityChange = (lineId: number, qty: number) => {
    setLines((prev) =>
      prev.map((l) => {
        if (l.sales_invoice_line_id !== lineId) return l;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (itemsToReturn.length === 0) {
      setError("يرجى تحديد كمية أكبر من الصفر لصنف واحد على الأقل لإتمام المرتجع.");
      return;
    }
    ...
    try {
      ...
      const result = await salesApi.createSalesReturn(payload);
      onSuccess(result);
      onClose();
    } catch (err: any) {
      setError(err.message || "حدث خطأ أثناء ترحيل مرتجع المبيعات.");
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <Modal
      isOpen={isOpen}
      onClose={onClose}
      title="إرجاع فاتورة مبيعات (إشعار دائن - Credit Note)"
      maxWidth="840px"
    >
      <div style={{ direction: "rtl", fontFamily: "inherit" }}>
        {loading ? (
          <div style={{ padding: "40px 20px", textAlign: "center", color: "#64748b" }}>
            <RotateCcw className="animate-spin" size={32} style={{ margin: "0 auto 12px", color: "#10b981" }} />
          ...
          <form onSubmit={handleSubmit}>
            ...
            >
              ...
              <div>
                <span style={{ fontSize: 11, color: "#64748b" }}>العميل</span>
                <div style={{ fontWeight: 700, fontSize: 13, color: "#1e293b" }}>
                  {invoiceData?.customer_name || "عميل نقدي عام"}
                </div>
              </div>

              <div>
                <span style={{ fontSize: 11, color: "#64748b" }}>تاريخ المرتجع</span>
                <input
                  type="date"
                  value={returnDate}
              ...
              <div>
                <span style={{ fontSize: 11, color: "#64748b" }}>طريقة رد المبلغ</span>
                <select
                  value={refundMethod}
            ...
            {!hasAnyRemaining ? (
              ...
              >
                ...
                <h4 style={{ margin: "0 0 6px", fontSize: 15 }}>تم إرجاع هذه الفاتورة بالكامل مسبقاً</h4>
                <div style={{ fontSize: 12, color: "#64748b" }}>
                  كافة الكميات في هذه الفاتورة تم عمل مرتجعات لها ولا توجد كميات متبقية للإرجاع.
                </div>
              </div>
            ) : (
              <>
                  ...
                  <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
                    <thead>
                      <tr style={{ background: "#f8fafc", borderBottom: "1px solid #e2e8f0", color: "#475569" }}>
                        ...
                        <th style={{ padding: "8px 12px", textAlign: "center" }}>المباع</th>
                            ...
                            <td style={{ padding: "10px 12px", textAlign: "center" }}>
                              {isExhausted ? (
                                ...
                                >
                                  <button
                                    type="button"
                                    onClick={() => handleQuantityChange(l.sales_invoice_line_id, l.quantity_to_return - 1)}
                                  ...
                                  <input
                                    ...
                                    onChange={(e) => {
                                      ...
                                      if (raw === "") {
                                        handleQuantityChange(l.sales_invoice_line_id, 0);
                                        return;
                                      }
                                      handleQuantityChange(l.sales_invoice_line_id, Number(raw) || 0);
                                    }}
                                    style={{
                                      width: 45,
                                      textAlign: "center",
                ...
                <div style={{ marginBottom: 16 }}>
                  ...
                  <input
                    ...
                    placeholder="مثال: تلف بالعبوة / استبدال صنف / رغبة العميل..."
                    style={{
                      width: "100%",
                      padding: "8px 12px",
                      borderRadius: 6,
            ...
            <div style={{ display: "flex", justifyContent: "flex-end", gap: 10 }}>
              ...
              {hasAnyRemaining && (
                ...
                >
                  <RotateCcw size={16} />
                  <span>{submitting ? "جاري ترحيل المرتجع..." : "تأكيد وترحيل المرتجع 🔄"}</span>
                </button>
              )}
            </div>
          </form>
